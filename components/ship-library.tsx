  ...
  return (
    <div className="bg-card p-6 rounded-lg border">
      ...
      <div className="space-y-3">
        {ships.map((ship) => (
          <div
            ...
            className={cn(
              "flex items-center gap-4 p-3 rounded-md border transition-all",
              {
                ...
                "bg-background border-border hover:bg-accent/10 cursor-grab":
                  !ship.placed,
              }
            )}
          ...
          >
            ...
            <div className="flex gap-1">
              {Array.from({ length: ship.size }, (_, i) => (
                <div
                  ...
                  className={cn(
                    "w-4 h-4 rounded-sm",
                    ship.placed ? "bg-primary/30" : "bg-primary"
                  )}
                />
              ))}
            </div>
            ...
            {ship.placed && (
              <div className="text-xs text-primary font-medium">Placed</div>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="mt-4 text-xs text-muted-foreground">
        Drag ships to the board to place them. Click the orientation button to
        rotate.
      </div>
    </div>
  );
