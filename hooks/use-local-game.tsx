...
export function LocalGameProvider({ children }: { children: React.ReactNode }) {
  const [players, setPlayers] = useState<[Player | null, Player | null]>([
    null,
    null,
  ]);
  ...
  const addPlayer = useCallback((name: string): number => {
    ...
    setPlayers((prev) => {
      if (!prev[0]) {
        playerNumber = 1;
        return [
          { id: Math.random().toString(36), name, ready: false },
          prev[1],
        ];
      } else if (!prev[1]) {
        playerNumber = 2;
  ...
  const placeShip = useCallback(
    (
      ...
      setGameState((prev) => {
        const newState = { ...prev };
        const board =
          playerNumber === 1 ? newState.player1Board : newState.player2Board;
        const ships =
          playerNumber === 1 ? newState.player1Ships : newState.player2Ships;
    ...
    setGameState((prev) => {
      ...
      const targetBoard =
        prev.currentPlayer === 1
          ? newState.player2Board
          : newState.player1Board;
      const targetShips =
        prev.currentPlayer === 1
          ? newState.player2Ships
          : newState.player1Ships;
